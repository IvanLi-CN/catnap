...
- 目标：对 `https://lazycats.online/cart` 的产品“配置（plan/config）”做库存监控与通知，避免手动频繁刷新页面。
...
- 不做多上游/多站点通用化（仅围绕 `lazycats.online/cart`）。
...
- 上游抓取策略（事实观察）：
  - `lazycats.online/cart` 页面包含“国家地区”“可用区域”“配置卡片”三层结构。
