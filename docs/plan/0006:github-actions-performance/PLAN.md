...
- [ ] M2: 引入 Bun/Playwright 缓存策略并验证命中（不改变既有检查口径）
- [ ] M3: 落地 PR gating + PR docker build 仅 `linux/amd64`（并补齐 buildx cache best-effort）
- [ ] M4: 文档同步（README/plan notes），确保贡献者可理解触发规则与排障入口
