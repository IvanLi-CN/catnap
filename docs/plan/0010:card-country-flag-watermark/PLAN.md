...
- 2026-01-25: 增加水印对角透明度渐变规范（左上 `~0.8` / 右下 `~0.1` 作为 mask 乘数）。
- 2026-01-25: 水印层贴边（距卡片边缘 `0`）；透明度渐变目标调整为 watermark layer alpha（左上 `~0.8` / 右下 `~0.1`）。
- 2026-01-25: 调整透明度渐变口径为 mask 乘数（左上 `~0.8` / 右下 `~0.1`），并明确水印不铺满整张卡片。
- 2026-01-25: 透明度渐变：右下目标改为 `0`（完全透明）。
