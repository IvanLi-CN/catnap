...
| 0006 | CI/CD：GitHub Actions 构建提速（PR：跳过 arm64 + gating + cache） | 部分完成（3/4） | `0006:github-actions-performance/PLAN.md` | 2026-01-22 | 冻结：PR 跳过 `linux/arm64`；前端重型检查允许 gating；Docker cache backend=`type=gha`；目标：PR ≤ 10 分钟（含 smoke + docker amd64） |
