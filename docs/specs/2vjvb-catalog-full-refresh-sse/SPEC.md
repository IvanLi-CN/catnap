...
- `docs/specs/README.md`: 增加本计划索引行
- `docs/specs/2vjvb-catalog-full-refresh-sse/ui/README.md`: 本计划 UI 设计说明
- `docs/specs/2vjvb-catalog-full-refresh-sse/ui/products.svg`: 本计划“全部产品”设计图
- `docs/specs/2vjvb-catalog-full-refresh-sse/ui/inventory-monitor.svg`: 本计划“库存监控”设计图
- `docs/specs/2vjvb-catalog-full-refresh-sse/ui/settings.svg`: 本计划“系统设置”设计图
- 实现阶段要求：完成 UI 改动后，将本计划目录内的 UI SVG **同步替换**到项目级集合 `docs/ui/`（并更新 `docs/ui/README.md` 的来源映射），确保工程内“最新设计图”不依赖 plan 目录。

## 实现里程碑（Milestones）

- [x] M1: 后端全量刷新 job（manual+auto）+ URL 级子任务去重/缓存（含持久化 last good）
- [x] M2: 生命周期差异（上架/下架/重新上架）+ DB 标记 + 通知开关接入（log + Telegram/Web Push）
- [x] M3: SSE 状态流 + 前端接入（按钮“立即刷新”/刷新中视觉态/错误提示）
- [x] M4: UI 下架标记 + 设置页新增配置项 + 完整测试与 Storybook 覆盖
...
- 2026-01-25: 完成实现与测试；“下架”标记改为标题内联展示（避免遮挡标题）
- 2026-01-25: 实现：全量刷新（manual+auto）+ SSE 状态流 + URL 任务去重/缓存 + 上/下架生命周期与通知 + UI 接入与测试

## 参考（References）

- UI：`docs/specs/2vjvb-catalog-full-refresh-sse/ui/README.md`
- UI：`docs/specs/2vjvb-catalog-full-refresh-sse/ui/products.svg`
- UI：`docs/specs/2vjvb-catalog-full-refresh-sse/ui/inventory-monitor.svg`
- UI：`docs/specs/2vjvb-catalog-full-refresh-sse/ui/settings.svg`
