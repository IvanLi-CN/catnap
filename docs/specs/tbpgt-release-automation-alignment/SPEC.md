# CI/CD：发版自动化（GHCR + GitHub Release + Release Assets）对标与补齐（#tbpgt）
...
- Given 有一个 PR 指向 `main`
  When CI 在 PR 上运行
  Then 会执行发布链路的构建校验（不 push），且 smoke test 通过
...
- Given 维护者在 GitHub Actions 手动触发 `workflow_dispatch`（可选选择 ref=main 或某个 tag）
  When CI 运行手动发布路径
  Then 会推送对应版本的镜像 tag（`v<semver>`）并上传 Release assets；且仅当 ref 为 `main` 时更新 `latest`
...
- 2026-01-20: 创建计划 #tbpgt，并冻结 release assets 矩阵、UI embed、单镜像与 major 级 pinning。
