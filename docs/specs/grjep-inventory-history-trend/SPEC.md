...
本计划新增接口与 DB schema；契约以本目录下文档为准，并与 #7ey9f（基础库存监控）的契约保持兼容。
...
- `docs/specs/grjep-inventory-history-trend/ui/README.md`
- `docs/specs/grjep-inventory-history-trend/ui/inventory-monitor-with-trend.svg`
- `docs/specs/grjep-inventory-history-trend/ui/products-with-trend.svg`
...
- `docs/specs/7ey9f-lazycats-cart-inventory-monitor/contracts/db.md`: 在实现阶段把“库存历史表”以增量变更方式同步进去，避免 DB 口径分裂
- `docs/specs/7ey9f-lazycats-cart-inventory-monitor/contracts/http-apis.md`: 若决定复用/扩展既有 endpoint（而非新增 endpoint），需同步更新该契约
...
- 本计划（#grjep）实现基线：以 `main`（当前为 `v0.1.4`）为准，在其上增量实现“历史库存 + 近 24h 走势”。
...
- `docs/specs/7ey9f-lazycats-cart-inventory-monitor/contracts/db.md`
- `docs/specs/7ey9f-lazycats-cart-inventory-monitor/contracts/http-apis.md`
