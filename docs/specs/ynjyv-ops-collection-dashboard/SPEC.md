...
- Given 本计划的 UI 设计图与设计文档已产出在 `docs/specs/ynjyv-ops-collection-dashboard/ui/`
  When 实现完成并准备交付
  Then 项目内不再存在对 `docs/specs/` 的运行/交付依赖，且下方 Asset promotion 已晋升到目标位置并完成引用更新。

## 实现前置条件（Definition of Ready / Preconditions）

- 本 `SPEC.md` 与契约文档已评审并冻结（接口字段与事件语义无需在实现阶段再改口径）。
...
- `docs/specs/README.md`: 本计划索引行与状态推进。
...
| --- | --- | --- | --- | --- | --- | --- |
| Ops 页面线框图（SVG） | `docs/specs/ynjyv-ops-collection-dashboard/ui/ops-dashboard.svg` | docs | copy | `docs/ui/ops-dashboard.svg` | `docs/ui/README.md` | 以“最新版本 UI 设计图”晋升到项目级目录（参照 `docs/ui/README.md` 约定） |
| SSE 状态悬浮气泡（SVG） | `docs/specs/ynjyv-ops-collection-dashboard/ui/sse-tooltip.svg` | docs | copy | `docs/ui/sse-tooltip.svg` | `docs/ui/README.md` | 仅用于解释“状态点 + 详情气泡”，不作为页面主体组成部分 |
...
- 相关计划：`2vjvb-catalog-full-refresh-sse`（同样涉及 SSE 与全局共享语义，本计划优先复用其基础设施而不改动其口径）。
