...
当前 `#machines` 页面需要在机器卡片层直接进入网页 VNC 控制台，但缓存里的 `panel_url`
只代表容器面板入口，不等于真实的控制台链接。若前端直接打开缓存地址，最终会落到 dashboard
而不是 `/console?token=...`。
...
- Storybook 与 API 测试覆盖“实时返回 console URL”及“无容器面板禁用”场景。
...
- [x] M4: 测试与 Storybook 覆盖实时 console URL 行为
