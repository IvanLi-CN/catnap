...
- 不在本计划中引入/实现“发版意图标签 gate”（该口径由 Plan #8btwa 承担）。
...
- Release 完整链路的提速与 runner 策略（另见 Plan #xrv7y）。
- 任意与 Release intent labels / gating 相关的行为变更（另见 Plan #8btwa）。
